<div align="center">
  <h1>🤖 AI/ML Mastery Roadmap</h1>
  <p><b>203-Day Comprehensive Journey from Zero to Job-Ready AI/ML Engineer</b></p>
  <p>
    <img src="https://img.shields.io/badge/Status-In%20Progress-blue?style=flat-square">
    <img src="https://img.shields.io/badge/Days%20Completed-10%20%2F%20203-brightgreen?style=flat-square">
    <img src="https://img.shields.io/badge/Language-Python%20%7C%20Jupyter-yellow?style=flat-square">
  </p>
</div>

---

## 📖 Overview

This repository tracks my complete AI/ML learning journey — from foundational statistics and data visualization to deep learning, NLP, Transformers, RAG, and full-stack AI deployment. Every day includes hands-on code, notebooks, and mini-projects.

## 📂 Structure

| Month | Days | Topics |
|-------|------|--------|
| [June](./June) | 1–19 | Seaborn, EDA, Data Cleaning, Titanic Capstone, Linear Regression Theory |
| [July](./July) | 20–50 | Linear Regression, Streamlit, Logistic Regression, KNN, K-Means, Decision Trees |
| [August](./August) | 51–81 | Random Forest, XGBoost, Optuna, SVM, Ensemble, ANN, CNN |
| [September](./September) | 82–111 | CNN, Transfer Learning, NLP, Naive Bayes, Sentiment Analysis |
| [October](./October) | 112–142 | RNN, LSTM, Transformers, BERT, GPT, RAG, Full Stack AI |
| [November](./November) | 143–172 | ML Pipelines, FastAPI, Docker, MLflow, Portfolio Building |
| [December](./December) | 173–203 | Revision, Mock Projects, Interview Prep, Final Polish |

## ✅ Progress

### June (Days 1–10) ✅
| Day | Topic | Notebook |
|-----|-------|----------|
| 1 | Seaborn Revision — distplot, boxplot, pairplot, heatmap | [📓](./June/Day%2001%20-%20Seaborn%20Revision.ipynb) |
| 2 | Seaborn Advanced — catplot, FacetGrid, themes, palettes | [📓](./June/Day%2002%20-%20Seaborn%20Advanced.ipynb) |
| 3 | EDA Intro — .info, .describe, .shape, dtypes, missing values | [📓](./June/Day%2003%20-%20EDA%20Intro.ipynb) |
| 4 | Handling Missing Values — mean/median/mode, dropna, fillna | [📓](./June/Day%2004%20-%20Handling%20Missing%20Values.ipynb) |
| 5 | Outlier Detection — IQR, Z-score, boxplot visualization | [📓](./June/Day%2005%20-%20Outlier%20Detection.ipynb) |
| 6 | Univariate Analysis — histograms, KDE, value_counts, skewness | [📓](./June/Day%2006%20-%20Univariate%20Analysis.ipynb) |
| 7 | Bivariate Analysis — scatterplots, correlation heatmap, groupby | [📓](./June/Day%2007%20-%20Bivariate%20Analysis.ipynb) |
| 8 | Multivariate Analysis — pairplot, pivot tables, crosstab | [📓](./June/Day%2008%20-%20Multivariate%20Analysis.ipynb) |
| 9 | Data Cleaning Pipeline — renaming, dtype conversion, dedup | [📓](./June/Day%2009%20-%20Data%20Cleaning%20Pipeline.ipynb) |
| 10 | EDA Capstone (Titanic) — full EDA workflow | [📓](./June/Day%2010%20-%20EDA%20Capstone%20Titanic.ipynb) |

### June (Days 11–19) 🔜
| Day | Topic |
|-----|-------|
| 11 | Feature Encoding — Label, One-Hot, Ordinal |
| 12 | Feature Scaling — StandardScaler, MinMaxScaler, RobustScaler |
| 13 | Handling Skewed Data — log, sqrt, Box-Cox |
| 14 | Feature Creation — binning, interaction, date features |
| 15 | Train/Test Split & Cross-Validation |
| 16 | Feature Engineering Capstone (Titanic) |
| 17 | Linear Regression Theory |
| 18 | Cost Function (MSE) |
| 19 | Gradient Descent |

### July (Days 20–50) 🔜
| Day | Topic |
|-----|-------|
| 20 | Linear Regression Implementation |
| 21 | Model Evaluation — R², MAE, RMSE |
| 22 | Multiple Linear Regression, VIF |
| 23 | Polynomial Regression |
| 24 | Ridge & Lasso Theory |
| 25 | Ridge & Lasso Implementation |
| 26 | Elastic Net |
| 27 | House Price Prediction Project |
| 28 | Streamlit Basics |
| 29 | Streamlit Input Widgets |
| 30 | Streamlit Project — Deploy House Price App |
| 31 | Logistic Regression Theory |
| 32 | Logistic Regression Cost Function |
| 33 | Logistic Regression Implementation |
| 34 | Evaluation Metrics — Confusion Matrix, Precision, Recall, F1 |
| 35 | ROC-AUC |
| 36 | SMOTE for Imbalanced Data |
| 37 | Titanic Project — Logistic Regression Pipeline |
| 38 | KNN Theory |
| 39 | Choosing K |
| 40 | KNN Implementation |
| 41 | KNN Evaluation |
| 42 | KNN Project |
| 43 | K-Means Clustering |
| 44 | Elbow Method & Silhouette |
| 45 | DBSCAN |
| 46 | PCA |
| 47 | Customer Segmentation Project |
| 48 | Decision Tree Theory |
| 49 | Decision Tree Construction |
| 50 | Decision Tree Overfitting |

### August (Days 51–81) 🔜
| Day | Topic |
|-----|-------|
| 51 | Decision Tree Implementation |
| 52 | Decision Tree Feature Importance |
| 53 | Decision Tree Project |
| 54 | Random Forest Theory |
| 55 | Random Forest Bootstrap |
| 56 | Random Forest Implementation |
| 57 | RF Hyperparameter Tuning |
| 58 | RF Feature Importance |
| 59 | Customer Churn Project |
| 60 | XGBoost |
| 61 | LightGBM |
| 62 | CatBoost |
| 63 | Optuna Basics |
| 64 | Optuna Advanced |
| 65 | SVM Theory |
| 66 | SVM Kernel Trick |
| 67 | SVM Project |
| 68 | Ensemble: Voting |
| 69 | AdaBoost Theory |
| 70 | AdaBoost Implementation |
| 71 | Stacking & Blending |
| 72 | Deep Learning Intro |
| 73 | ANN Layers |
| 74 | Activation Functions |
| 75 | Forward Propagation |
| 76 | Loss Functions |
| 77 | Backpropagation |
| 78 | ANN Implementation (Keras) |
| 79 | Optimizers |
| 80 | Regularization |
| 81 | ANN Project — Churn Classifier |

### September (Days 82–111) 🔜
| Day | Topic |
|-----|-------|
| 82 | model.save() / load_model() |
| 83 | CNN Theory |
| 84 | Padding & Pooling |
| 85 | Feature Maps |
| 86 | CNN Architecture |
| 87 | CNN Implementation |
| 88 | Data Preprocessing |
| 89 | Data Augmentation |
| 90 | CNN Training — MNIST |
| 91 | CNN Evaluation |
| 92 | Transfer Learning |
| 93 | Digit Classifier Project |
| 94 | NLP Basics |
| 95 | Text Preprocessing |
| 96 | Stopwords & Stemming |
| 97 | Bag of Words |
| 98 | Cosine Similarity |
| 99 | N-grams |
| 100 | Word Embeddings |
| 101 | TF-IDF Theory |
| 102 | TF-IDF Implementation |
| 103 | TF-IDF Comparison |
| 104 | Naive Bayes |
| 105 | Naive Bayes Project — Spam Detection |
| 106 | Sentiment Analysis Overview |
| 107 | Sentiment: Cleaning |
| 108 | Sentiment: Feature Extraction |
| 109 | Sentiment: Model Training |
| 110 | Sentiment: Evaluation |
| 111 | Sentiment Analysis Project |

### October (Days 112–142) 🔜
| Day | Topic |
|-----|-------|
| 112 | RNN Theory |
| 113 | LSTM Theory |
| 114 | LSTM Code |
| 115 | Attention Mechanism |
| 116 | Transformer Basics |
| 117 | Hugging Face Ecosystem |
| 118 | BERT |
| 119 | GPT |
| 120 | RAG |
| 121 | Vector Databases |
| 122 | RAG Project — Chat With PDF |
| 123 | Ollama |
| 124–135 | Full Stack AI — Architecture, FastAPI, React, MongoDB, Auth, Deployment, Testing, Docs |

### November (Days 143–172) 🔜
| Day | Topic |
|-----|-------|
| 143–148 | ML Pipelines, FastAPI for ML, Docker, MLflow, Monitoring, CI/CD |
| 149–152 | Time Series — ARIMA, Prophet |
| 153–155 | GANs, Autoencoders, RL |
| 156–158 | Feature Selection, Model Calibration |
| 159–163 | Portfolio Building |
| 164–172 | ML Revision (4 weeks) + Mock Project Reviews (5 projects) |

### December (Days 173–203) 🔜
| Day | Topic |
|-----|-------|
| 173 | Mock Project Review 6 — Full Stack AI |
| 174–176 | Interview Prep — ML Theory, DL Theory, STAR Format |
| 177–203 | Final Review, Portfolio Polish, Buffer Revision, Mock Tests, Final Checks, Celebration 🎉 |

## 📊 Learning Resources

| Resource | Description |
|----------|-------------|
| [AI_Mastery_Blueprint.pptx](./AI_Mastery_Blueprint.pptx) | Full roadmap presentation |
| [AI_Mastery_Blueprint.pdf](./AI_Mastery_Blueprint.pdf) | PDF version of the blueprint |
| [roadmap.txt](./roadmap.txt) | Complete day-wise topic list |

## 🛠️ Tech Stack

| Category | Tools |
|----------|-------|
| **Languages** | Python |
| **Data Analysis** | Pandas, NumPy |
| **Visualization** | Seaborn, Matplotlib, Plotly |
| **ML Framework** | Scikit-Learn, XGBoost, LightGBM, CatBoost |
| **Deep Learning** | TensorFlow, Keras |
| **NLP** | NLTK, spaCy, Hugging Face Transformers |
| **LLM/RAG** | LangChain, ChromaDB, FAISS, Ollama |
| **Web Framework** | FastAPI, Streamlit, React |
| **MLOps** | Docker, MLflow, GitHub Actions |
| **Database** | MongoDB |

## 📈 Progress Tracker

```
June    ████████████████████░░░░░░░░░░░░░░░░░░░  10/203 days
July    ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
August  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
September ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
October ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
November ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
December ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░   0/203 days
```

---

<div align="center">
  <b>🚀 203 Days → 203 Days Ready 🚀</b>
  <br>
  <i>Built one notebook at a time</i>
</div>
